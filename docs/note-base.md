...
```text
[完整主线 —— 从「公网怎么进」到「入口长什么样」]
  │
  ▼
① 问题：多服务多端口 + 单一公网 IP，怎么对外暴露？
   ├─▶ 方案A：公网端口一一映射（每本地端口开一个公网端口）
   │     → 端口多难记、安全组暴露面大、公网端口数量有限 ✖
   └─▶ 方案B：一个公网端口 + 多路径分发（反向代理网关）✔ 采纳
        统一入口 http://<公网IP>/<路径>
  ▼
② 落地卡点：路径分发不是所有服务都接得住
   · 无域名 → 只能 HTTP；Caddy 自动 HTTPS 需域名，有域名后切 :443 + 子域名
   · 子路径感知：Gitea 配 ROOT_URL、Grafana 配 root_url + serve_from_sub_path、
     Registry UI 相对路径剥前缀即可
   · 例外：new-api 不支持子路径 → 走独立端口映射
   · 转发模式：handle（保前缀，后端需感知子路径）vs handle_path（剥前缀，后端以为挂根路径）
  ▼
③ 网关定型：Caddy 监听 :80（内网直连）+ :8080（公网 2029→8080），规则一致
   路径分发 7 服务：/gitea /grafana /prom /registry /solar /webui /api
   兜底 / → 静态导航首页（hao123 风格）
  ▼
④ 首页布局迭代
```
...
- **一个公网端口 + 路径分发 vs 一一映射**：前者只开一个端口、统一入口（`http://<IP>/<路径>`）、暴露面小、好记，代价是要求应用支持子路径；后者端口爆炸、难记难管——本机最终采用前者
- **子路径兼容性（为什么有的能挂路径、有的只能跳转）**：反代子路径成败 = 应用页面生成的资源/跳转链接是否带前缀。挂在 `/xxx` 下后，浏览器请求页面里的资源都会落在 `/xxx/` 之下：资源用相对路径（`assets/app.js`）→ 自动解析到 `/xxx/assets/...` ✅；写死绝对路径（`/assets/...`、`/static/...`）→ 打到网关根路径 → 按路径分发匹配不到 → 404，页面样式/功能崩，除非应用支持配置 base path。实测各服务形态：


  | 服务               | 子路径模式                                                       | Caddy 转发            | 首页入口                         |
  | ------------------ | ---------------------------------------------------------------- | --------------------- | -------------------------------- |
  | Gitea              | 根路径服务 +`ROOT_URL=http://<IP>/gitea`（仅用于生成带前缀链接） | `handle_path` 剥前缀  | `/gitea`                         |
  | Grafana            | `root_url` + `serve_from_sub_path=true`（自身感知子路径）        | `handle` 保前缀       | `/grafana`                       |
  | Prometheus         | `--web.route-prefix=/prom`                                       | `handle` 保前缀       | `/prom`                          |
  | Registry UI        | 资源全相对路径                                                   | `handle_path` 剥前缀  | `/registry`                      |
  | solar / vLLM WebUI | 感知子路径（已配反代）                                           | `handle` 保前缀       | 首页无 solar 卡 / WebUI 跳`8081` |
  | new-api            | 不支持子路径（Caddyfile 注释明确「不支持子路径改独立端口」）     | `/api` 保留但入口不走 | 跳`18080` 独立端口               |
  | Portainer          | UI 写死绝对路径、官方不支持反代子路径                            | 无                    | 跳`9443` 独立端口（仅内网）      |

  **接入判据**：新服务先查有无 `root_url` / `base_path` / `route-prefix` 类配置——有则挂路径（统一入口）；没有且资源写死绝对路径 → 只能独立端口跳转。注意「有路径反代」≠「首页入口走路径」：`/api`、`/webui` 虽配了反代，首页入口仍跳独立端口（实际体验 / 兼容性取舍）
- **无域名限制**：IP 直连场景无 HTTPS（Caddy 自动 HTTPS 需域名）；有域名后再切 `:443` + 子域名分发，证书数据存 caddy_data 卷
- **保前缀 / 剥前缀**：`handle` 保留前缀给后端（后端需配子路径），`handle_path` 剥掉前缀（后端以为自己挂根路径）——按服务对子路径的感知能力选模式
